...
const AddEditCampaign = ({
  ...
  return (
    <>
      ...
      <PageTitle title={isEditView ? "Edit Campaign" : "Add Campaign"} />
      <Form
        form={form}
      ...
      >
        <Row className="fields-row" gutter={20} type="flex">
          <Col span={8} xs={24} sm={12} lg={12}>
            ...
            <Form.Item label="Campaign Title" name="campaignName" rules={[{ required: true }]}>
          ...
          <Col span={8} xs={24} sm={12} lg={12} className="mg-top-40">
            ...
            <Form.Item label="Current Status" className="text-left" name="status">
              <Radio.Group className="campaign__radio-group">
                {_map(currentStatus, value => (
                  <Radio.Button key={value} onClick="return false;" className="campaign__radio-button" value={value}>
                    {value}
                  </Radio.Button>
                ))}
              </Radio.Group>
        ...
        <Row className="fields-row" gutter={20} type="flex">
          <Col span={8} xs={24} sm={12} lg={12}>
            <Form.Item label="Location Name" name="locationName" rules={[{ required: true }]}>
              <Input placeholder="Location Name" />
            </Form.Item>
            <Form.Item label="Area Segment" name="areaSegment" rules={[{ required: true }]}>
              <Input placeholder="Area Segment" />
            </Form.Item>
            <Form.Item label="Location" name="location">
              <a href="#">(Google Map Pin)</a>
            </Form.Item>
            <Form.Item label="Transferable" name="isTransferable">
              <Radio.Group>
          ...
          <Col span={8} xs={24} sm={12} lg={12}>
            {isEditView ? (
              <div className="campaign__promotion-count">
                <p>Categories in promotion: {_get(campaign, "categoriesCount", "0")}</p>
                <p>Brands in promotion: {_get(campaign, "brandsCount", "0")}</p>
                <p>Vouchers in promotion: {_get(campaign, "vouchersCount", "0")}</p>
              </div>
            ) : null}
            {isEditView ? (
              <Form.Item className="mg-top-40">
                <Button type="primary" onClick={() => setPromotionDrawerVisible(true)}>
                  Manage Promotions
                </Button>
              </Form.Item>
            ) : null}
        ...
        <AddRemoveDealProvider
          visible={promotionDrawerVisible}
          close={onPromotionDrawerClose}
